...
import com.xht.inspirahivebackend.api.aliyunai.model.CreateOutPaintingTaskRequest;
...
import com.xht.inspirahivebackend.manager.CosManager;
import com.xht.inspirahivebackend.manager.upload.FilePictureUpload;
import com.xht.inspirahivebackend.manager.upload.PictureUploadTemplate;
import com.xht.inspirahivebackend.manager.upload.UrlPictureUpload;
import com.xht.inspirahivebackend.model.dto.file.UploadPictureResult;
...
import com.xht.inspirahivebackend.mapper.PictureMapper;
import com.xht.inspirahivebackend.service.SpaceService;
import com.xht.inspirahivebackend.service.UserService;
import com.xht.inspirahivebackend.utils.ColorSimilarUtils;
import com.xht.inspirahivebackend.utils.ColorTransformUtils;
...
//    private FileManager fileManager;
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (spaceId != null) {
            ...
            if (space.getTotalCount() >= space.getMaxCount()) {
                throw new BusinessException(ErrorCode.OPERATION_ERROR, "空间图片数量已达到上限");
            }
    ...
    public List<PictureVO> searchPictureByColor(Long spaceId, String picColor, User loginUser) {
        // 1.校验参数
        ThrowUtils.throwIf(spaceId == null || StringUtils.isBlank(picColor), ErrorCode.PARAMS_ERROR);
        ThrowUtils.throwIf(loginUser == null, ErrorCode.NO_AUTH_ERROR);
        // 2.校验空间权限
        Space space = spaceService.getById(spaceId);
        ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不存在");
        ...
        List<Picture> sortedPictureList = pictureList.stream()
                .sorted(Comparator.comparingDouble(picture -> {
                    String listPicColorStr = picture.getPicColor();
                    if (listPicColorStr == null) {
                        return Double.MAX_VALUE;
                    }
                    Color listPicColor = Color.decode(listPicColorStr);
                    // 计算相似度
                    return -ColorSimilarUtils.calculateSimilarity(targetColor, listPicColor);
                }))
                .limit(12)
    ...
    public void editPictureByBatch(PictureEditByBatchRequest pictureEditByBatchRequest, User loginUser) {
        ...
        List<String> tag = pictureEditByBatchRequest.getTags();
        // 1.参数校验
        ...
        pictureList.forEach(picture -> {
            ...
            if (CollUtil.isNotEmpty(tag)) {
                picture.setTags(JSONUtil.toJsonStr(tag));
            }
        });
        // 批量重命名
